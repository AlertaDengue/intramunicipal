      ...
      - text: |
          ...
          \fancyhead[L]{%
            \begin{minipage}{\textwidth}
              \includegraphics[height=40pt]{~/Documentos/intramunicipal/logo1.png}%
              \hfill%
              \includegraphics[height=40pt]{~/Documentos/intramunicipal/logo2.png}\\[-6pt]
          ...
          \fancyfoot[L]{%
            \begin{minipage}{\textwidth}
              ...
              \hfill Boletim Municipal - Joinville \hfill SE \theweek\ de \theyear
            \end{minipage}
          }
...
load("~/Documentos/intramunicipal/joinville/ref2_JV.RData")
...
# dados_api_dengue <- read.csv("./dados_novos/se49/dengue_52-49.csv") %>% 
#   mutate(arbo = "dengue") %>% 
#   arrange(SE)
# 
# dados_api_chikon <- read.csv("./dados_novos/se49/chikungunya_52-49.csv") %>% 
#   mutate(arbo = "chikon") %>% 
#   arrange(SE)

## SUBSTITUIR AQUI POR DADOS da tabela_historico_alerta
dados_api_dengue <- read.csv("~/Documentos/dados_teste_intramunicipal/joinville/se05/dengue_52-5.csv") %>%
  mutate(arbo = "dengue") %>%
  arrange(SE)

## SUBSTITUIR AQUI POR DADOS da tabela_historico_alerta
dados_api_chikon <- read.csv("~/Documentos/dados_teste_intramunicipal/joinville/se05/chikungunya_52-5.csv") %>%
  mutate(arbo = "chikon") %>%
  arrange(SE)

dados_api <- rbind(dados_api_dengue, dados_api_chikon)

## SUBSTITUIR AQUI POR DADOS da tabela_notificação
# Dados de notificacoes
df_mun <- read_parquet("~/Documentos/dados_teste_intramunicipal/joinville/se05/dengue.parquet") %>% 
  filter(municipio_geocodigo == 4209102) %>% 
  mutate(
...
df_bairros <- df_mun_ok %>% 
  ...
  left_join(jv_shp, by = c("nm_bairro_ref" = "nome_bairr")) %>% 
  left_join(res_distritos_nomes, by = c("nm_bairro_ref" = "nome_bairr")) %>% 
...
nowcastdg_distritos <- df_mun_ok %>% 
  filter(arbo == "dengue") %>% 
  left_join(res_distritos_nomes %>% select(nome_bairr, distrito),
            by = c("nm_bairro_ref" = "nome_bairr")) %>% 
  select(dt_digita, dt_sin_pri, dt_notific, distrito)
```

```{r calcular_alertas, include=FALSE, cache=FALSE}
...
df_distritos <- df_distritos %>% 
  ...
  mutate(
    id_distrito_nome = case_when(
      distrito == "Distrito Centro" ~ 1,
      distrito == "Distrito Norte" ~ 2,
      distrito == "Distrito Sul" ~ 3
    ),
...
ids_distrito_nome <- unique(df_distritos$id_distrito_nome)
nomes_distrito_nome <- unique(df_distritos$distrito)

df_distrito_dengon <- map2_dfr(
  ids_distrito_nome, nomes_distrito_nome, calcular_alerta,
...
if (any(df_distritos$arbo == "chikon", na.rm = TRUE)) {
  ...
  df_distritos <- rbind(df_distrito_dengon, df_distrito_chikon)
} else {
  df_distritos <- df_distrito_dengon
}
...
```{r nowcasting, include=FALSE, cache=FALSE}
...
# Nowcasting por distrito
# AJUSTAR: Populações reais das RPAs de Recife
populacoes_distrito_nomes <- jv_pop %>% 
  left_join(res_distritos_nomes %>% select(nome_bairr, distrito), 
            by = c("nome_bairr" = "nome_bairr")) %>% 
  group_by(distrito) %>% 
...
rf_distrito_nomes_nowcast <- list()

# Processar cada RPA
for (distrito in nomes_distrito_nome) {
  
  # Filtrar dados para o RPA específico
  dados_distrito_filtrado <- nowcastdg_distritos %>%
    filter(distrito == .env$distrito)
  
  # Nowcasting para o RPA
  distrito_nowscasting <- nowcasting_inla(
    dataset = dados_distrito_filtrado,
    data.by.week = TRUE,
    date_onset = dt_sin_pri,
    date_report = dt_digita,
    K = 2, 
  ...
  distrito_observados <- dados_distrito_filtrado %>%
    mutate(
      dt_event = dt_sin_pri,
      epiweek = epical::epi_week(dt_event)[[1]],
      ano = year(dt_event),
  ...
  # Processar nowcast
  distrito_nowcast <- distrito_nowscasting$total %>%
    mutate(
      ano = year(dt_event),
      epiweek = epical::epi_week(dt_event)[[1]],
      ano_epi = case_when(
  ...
  rf_distrito_nomes_nowcast[[distrito]] <- list(
    nowcast = list(total = distrito_nowcast),
    observado = distrito_observados
  )
}
```
...
rf_inc_mun_dengon <- create_incidence_plot( 
  ...
  title_suffix = paste0("Curva de incidência de dengue - Joinville até SE", week_atual),
  include_zoom = FALSE, 
  convert_to_incidence = TRUE, 
  pop = 616317  # População de Recife (IBGE 2022)
)
...
rt_mun_dengon <-create_rt_plot(
  api_data = dados_rt_mun_dengue %>% 
    rename(SE = sem_not),
  weeks_limit = weeks_2_plot[3], 
  title_suffix = paste0("Curva de Rt dengue - Joinville até ", week_atual),
...
if (any(dados_api$arbo == "chikon", na.rm = TRUE)) {
 ...
 rt_mun_chikon <-  create_rt_plot(
  api_data = dados_rt_mun_chikon %>% 
    rename(SE = sem_not),
  weeks_limit = weeks_2_plot[3], 
  title_suffix = paste0("Curva de Rt chikungunya - Joinville até ", week_atual),
  sem_not = "SE", lwr = "lwr", upr = "upr"
)
...
# Rt por distrito - Dengue
rt_distrito_nome_dengon <- create_rt_plot(
...
# Rt por distrito - Chikungunya
if (any(dados_api$arbo == "chikon", na.rm = TRUE)) {
  rt_distrito_nome_chikon <- create_rt_plot(
    api_data = df_distritos %>% 
  ...
  rt_distrito_nome_chikon <- NULL
}

# Incidencia por distrito - Dengue
rf_inc_distrito_nomes_dengon <- map(names(rf_distrito_nomes_nowcast), ~ {
  distrito_nome <- .x
  create_incidence_plot(
    nowcast_data = rf_distrito_nomes_nowcast[[distrito_nome]]$nowcast$total,
    observed_data = rf_distrito_nomes_nowcast[[distrito_nome]]$observado %>% filter(ano_epi <= ano_week),
    distrito_nome = distrito, 
    convert_to_incidence = TRUE,
    pop = populacoes_distrito_nomes[distrito_nome],
    include_zoom = FALSE, 
    title_suffix = distrito_nome
  )
})

# Incidencia por distrito - Chikungunya
if (any(df_distritos$arbo == "chikon", na.rm = TRUE)) {
  min_val <- min(df_distritos %>% filter(arbo == "chikon") %>% pull(inc), na.rm = TRUE)
  max_val <- max(df_distritos %>% filter(arbo == "chikon") %>% pull(inc), na.rm = TRUE)
  
  rf_inc_distrito_nomes_chikon <- df_distritos %>% 
    filter(arbo == "chikon" & sem_not >= ano_week-100) %>% 
  ...
  rf_inc_distrito_nomes_chikon <- NULL
}
...
# Mapas - distritos
rf_map_distrito_nomes_dengon <- create_map_panel(
  generate_maps(
    data = df_distritos %>%
      left_join(jv_distritos_shp %>% select(distrito,geometry)), 
    value_col = "inc",
    arbo = "dengue",
...
if (any(df_distritos$arbo == "chikon", na.rm = TRUE)) {
  rf_map_distrito_nomes_chikon <- create_map_panel(
    generate_maps(
      data = df_distritos %>%
      left_join(jv_distritos_shp %>% select(distrito,geometry)), 
      value_col = "inc",
  ...
  rf_map_distrito_nomes_chikon <- NULL
}
...
# Tabela 3 - distritos Dengue
casos_acumulados_distrito_nome_dengon <- df_distritos %>% 
...
tab3_dados <- df_distritos %>% 
  ...
  mutate(
    ...
    rpa_nome = factor(distrito)
  ) %>% 
  left_join(casos_acumulados_distrito_nome_dengon %>% 
...
tab3 <- tab3_dados %>%
  ...
  kable_styling(latex_options = c("HOLD_position", "striped"),
                font_size = 10,
                position = "center") %>%
  pack_rows(index = table(tab3_dados$distrito)) %>%
...
casos_acumulados_distrito_nome_chikon <- df_distritos %>% 
...
tab4_dados <- df_distritos %>%
  ...
  left_join(casos_acumulados_distrito_nome_chikon %>% 
              select(sem_not, distrito, acumulado)) %>% 
  ungroup() %>%
  arrange(distrito, desc(sem_not)) %>%
  select(sem_not, acumulado, casos_notificados, inc, nivel, distrito)
...
tab4 <- tab4_dados %>%
  ...
  kable_styling(latex_options = c("HOLD_position", "striped"),
                font_size = 10,
                position = "center") %>%
  pack_rows(index = table(tab4_dados$distrito)) %>%
...
# Situação das arboviroses em Joinville - SC
...
```{r fig, fig.cap=paste0("Número de dados notificados e perdidos de dengue (A) e chikungunya (B) até a SE",week_atual,"."), fig.width=6, fig.height=7, out.width= "65%"}
fig4
```

## Análise da situação das arboviroses

Esse boletim analisa as condições de transmissão das arboviroses utilizando dados de clima (Copernicus ERA5) e de notificação de casos (SINAN) fornecidos semanalmente pela CGARB/SVS/MS. A partir desses dados são analisadas as condições de receptividade climática, transmissão e incidência (ver descrição no final do relatório), tendo como objetivo contribuir para a tomada de decisão na sala de situação. A seguir, são apresentados os resultados das análises em três níveis espaciais: nível do município, distritos.
...
```{r fig6, fig.cap=paste0("Curva de incidência de Dengue (A) e Chikungunya (B) - Recife/PE até SE", week_atual, ". A linha azul representa o número de casos observados e a linha vermelha horizontal o limiar epidêmico (incidência superior a 27 casos por 100 mil habitantes)."), fig.width=7, fig.height=6, out.width="85%"}
ggarrange(
  rf_inc_mun_dengon,
  rf_inc_mun_chikon,
  ncol = 1, 
...
```{r fig8, fig.cap=paste0("Número reprodutivo semanal de Dengue (a) e Chikungunya (b) para o município estimado até SE", week_atual, "."), fig.width=5, fig.height=7, out.width= "70%"}
ggarrange(
...
A tabela abaixo sumariza em relação às últimas 3 semanas epidemiológicas (`r ultimas_3_semanas`) e para cada Distrito de Saúde, o número de casos observados, casos estimados, a incidência por 100 mil habitantes, o total de casos acumulados e o nível de alerta calculados.

**Tabela 4:** Número de casos observados, casos estimados, incidência, receptividade climática e nível de alerta para as últimas três semanas, por distrito.
...
A figura 9 mostra o perfil de incidência de dengue nos distritos e as cores indicam o nível de atenção da semana epidemiológica. O código de cores indica o nível de alerta da semana epidemiológica.
...
# plot_grid(
#   rf_inc_distrito_nomes_dengon[[1]], #+ ylim(0,500),
#   rf_inc_distrito_nomes_dengon[[2]], #+ ylim(0,500),
#   rf_inc_distrito_nomes_dengon[[3]], #+ ylim(0,500),
#   ncol = 1
# )

plot_grid(plotlist = rf_inc_distrito_nomes_dengon, ncol = 2)
```

\vspace{1cm}

A figura 10 ilustra a incidência por 100 mil habitantes dos casos notificados de dengue agregados pelos Distrito de Saúde em Joinville, entre as semanas epidemiológicas `r ultimas_3_semanas` de `r ano_atual`. Para cada semana é apresentada a porcentagem de casos notificados que não puderam ser alocados em nenhum Distrito de Saúde devido a problemas no preenchimento ou ausência de informação sobre o local de ocorrência dos casos.
...
**Tabela 4:** Número de casos observados, incidência, receptividade climática e nível de alerta para Chikungunya nas últimas três semanas, por distrito.

```{r}
tab4
```

A figura 11 mostra o perfil de incidência de chikungunya nos Distritos de Saúde e as cores indicam o nível de atenção da semana epidemiológica.

```{r fig11, fig.cap=paste0("Curva de incidência de chikungunya para os RPAs Centro, Norte e Sul até SE", week_atual, ". A linha azul representa o número de casos notificados e a linha vermelha horizontal o limiar epidêmico (incidência superior a 27 casos por 100 mil habitantes)."), fig.width=12, fig.height=12, out.width="100%", out.height="110%"}

# plot_grid(
#   rf_inc_distrito_nomes_chikon[[1]], #+ ylim(0,500),
#   rf_inc_distrito_nomes_chikon[[2]], #+ ylim(0,500),
#   rf_inc_distrito_nomes_chikon[[3]], #+ ylim(0,500),
#   ncol = 1
# )

plot_grid(plotlist = rf_inc_distrito_nomes_chikon, ncol = 2)

```

A figura 12 ilustra a incidência por 100 mil habitantes dos casos notificados de chikungunya agregados pelos Distrito de Saúde em Joinville, entre as semanas epidemiológicas `r ultimas_3_semanas` de `r ano_atual`.
...
rf_map_distrito_nomes_chikon
```

## Perfil sazonal da transmissão das Arboviroses

O perfil sazonal das arboviroses está representado nos gráficos abaixo. O perfil sazonal da receptividade climática para os Distritos de Saúde apresenta uma escala que varia de 0 (período pouco receptivo) a 100 (período muito receptivo) sendo que, períodos muito receptivos, marcam a sazonalidade da doença.

O perfil de transmissibilidade para estas arboviroses é medido pelo número reprodutivo (Rt), calculado para cada distrito. Valores dessa métrica maiores que 1 ou mais de 3 semanas seguidas indicam transmissão sustentada de dengue ou chikungunya.
...
rt_coluna_dengue <- ggarrange(
  rt_distrito_nome_dengon$individual_plots$`Distrito Centro` + rremove("ylab") + rremove("xlab"),
  rt_distrito_nome_dengon$individual_plots$`Distrito Norte` + rremove("ylab") + rremove("xlab"),
  rt_distrito_nome_dengon$individual_plots$`Distrito Sul` + rremove("ylab") + rremove("xlab"),
  nrow = 3,
  labels = letters[1:3],
...
rt_coluna_chik <- ggarrange(
  rt_distrito_nome_chikon$individual_plots$`Distrito Centro` + rremove("ylab") + rremove("xlab"),
  rt_distrito_nome_chikon$individual_plots$`Distrito Norte` + rremove("ylab") + rremove("xlab"),
  rt_distrito_nome_chikon$individual_plots$`Distrito Sul` + rremove("ylab") + rremove("xlab"),
  nrow = 3,
  labels = letters[4:6],
  font.label = list(size = 12, face = "bold")
) %>% 
