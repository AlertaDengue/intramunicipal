...
processar_nowcast_rpa <- function(rpas_nomes, dados_rpas, K = NULL, Dmax = NULL)  {
  # Preparar dados
  # dados_rpa <- nowcastdg_rpa %>% 
  #   left_join(df_bairros %>%
  #               select(nm_bairro_ref, rpa_nome) %>% 
  #               distinct, 
  #             by = "nm_bairro_ref") %>% 
  #   select(nu_notific,sem_not,dt_digita,dt_sin_pri,rpa_nome) %>% 
  #   #na.omit() %>% 
  #   filter(rpa_nome == rpa_nome_especifica,
  #          year(dt_sin_pri) %in% c(ano_atual-2, ano_atual-1, ano_atual)) %>%
  #   select(dt_sin_pri, dt_digita, rpa_nome)
  
  # dados_rpa[dados_rpa == ""] <- NA
  # dados_rpa <- na.omit(dados_rpa)
  
  dados_rpa_filtrados <- dados_rpas %>%
    filter(rpa_nome == rpas_nomes)
  
  # Gerar nowcasting
  df_nowcast <- nowcasting_inla(
    dataset = dados_rpa_filtrados,
    data.by.week = TRUE,
    date_onset = dt_sin_pri,
    date_report = dt_digita,
    K = K, Dmax = Dmax
  ...
  # Dados observados por semana
  dados_by_week <- dados_rpa_filtrados %>% 
    mutate(
      dt_event = dt_sin_pri,
      epiweek = epical::epi_week(dt_event)[[1]],
      ano = year(dt_event),
