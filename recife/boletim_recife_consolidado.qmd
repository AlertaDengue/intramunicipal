      ...
      - text: |
          ...
          \fancyfoot[L]{%
            \begin{minipage}{\textwidth}
              ...
              \hfill Boletim Municipal - Recife \hfill SE \theweek\ de \theyear
            \end{minipage}
          }
...
dados_api_dengue <- read.csv("~/Documentos/dados_teste_intramunicipal/recife/se05/dengue_52-5.csv") %>% 
  mutate(arbo = "dengue") %>% 
  arrange(SE)

dados_api_chikon <- read.csv("~/Documentos/dados_teste_intramunicipal/recife/se05/chikungunya_52-5.csv") %>% 
  mutate(arbo = "chikon") %>% 
  arrange(SE)
...
df_mun <- read_parquet("~/Documentos/dados_teste_intramunicipal/recife/se05/dengue.parquet") %>% 
...
# Situação das arboviroses em Recife - PE
...
plot_grid(plotlist = rf_inc_rpa_nomes_dengon, ncol = 2)
```
...
plot_grid(plotlist = rf_inc_rpa_nomes_chikon, ncol = 2)
```
