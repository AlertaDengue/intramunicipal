  ...
  pdf:
    ...
    include-in-header:
      text: |
        ...
        \pagestyle{fancy}
        \fancyhf{}
        \fancyhead[L]{\includegraphics[height=40pt]{logo1.png}}
        \fancyhead[R]{\includegraphics[height=40pt]{logo2.png}}
...
df_mun <- read_parquet("./dados_novos/se49/dengue.parquet") %>% 
  filter(municipio_geocodigo == 5300108) %>% 
  mutate(
  ...
  left_join(df_ref, by = c("id_distrit" = "ID_DISTRITO"))
...
week_atual <- as.numeric(substr(weeks_2_plot_full[3], 5, 6))
ano_atual <- as.numeric(substr(weeks_2_plot_full[3], 1, 4))
...
df_bairros <- df_mun_filtrado %>% 
  count(arbo, RA, sem_not, name = "notificações") %>%
  mutate(sem_not = as.numeric(sem_not)) %>% 
  completar_dados_bairros(
    bairros_unicos = unique(df_ref$RA),
    se_var = "sem_not",
    nm_bairr_var = "RA",
    group_var = "arbo",
    max_semana_epidemiologica = weeks_2_plot_full[3]
  ) %>%
  left_join(df_ref %>% select(-ID_DISTRITO), by = c("RA" = "RA")) %>%
  left_join(df_shp, by = c("RA" = "RA")) %>% 
  left_join(df_regioes_shp %>% select(-ID_DISTRITO), by = c("RA" = "RA")) %>% 
...
df_mun_filtrado %>% 
  left_join(df_regioes_shp, by = "RA")
...
mun_nowcast <- mun_nowscasting$total %>%
  mutate(
    ...
    type = case_when(
      ano_epi > as.numeric(as.character(weeks_2_plot_full[3])) ~ 'Forecasting',
      TRUE ~ "Nowcasting"
    )
  )
...
bairros_ultima_semana_dengue$semanas_com_nivel3_4 <- sapply(
  ...
  function(bairro) calcular_semanas_alerta_alto(
    ...
    weeks_2_plot_full[3], bairro, janela = 5
  )
)
...
bairros_ultima_semana_chikon$semanas_com_nivel3_4 <- sapply(
  ...
  function(bairro) calcular_semanas_alerta_alto(
    ...
    weeks_2_plot_full[3], bairro, janela = 10
  )
)
...
fig4A <- perda_notificação %>% 
  filter(sem_not >= paste0(ano_atual-1, substr(weeks_2_plot_full[3], 5, 6)) &
           arbo == "dengue") %>% 
  ggplot() + 
  ...
  scale_x_discrete(breaks = levels(factor(df_mun$sem_not))[
    seq(1, length(levels(factor(df_mun$sem_not))), by = 8)]) + 
  theme_minimal(base_size = 11) +
  theme(axis.text.x = element_text(angle = 60, hjust = 1),
        legend.position = "top")
...
rt_distrito_nome_dengon <- create_rt_plot(
  api_data = df_distritos %>% 
    filter(arbo == "dengue" & sem_not >= paste0(ano_atual-1, week_atual)), 
  weeks_limit = weeks_2_plot_full[3], 
  facet_by = "distrito",
  title_suffix = "",
  lwr = "lwr", upr = "upr"
...
if (any(dados_api$arbo == "chikon", na.rm = TRUE)) {
  rt_distrito_nome_chikon <- create_rt_plot(
    api_data = df_distritos %>% 
      filter(arbo == "chikon" & sem_not >= paste0(ano_atual-1, week_atual)), 
    weeks_limit = weeks_2_plot_full[3], 
    facet_by = "distrito",
    title_suffix = "",
    lwr = "lwr", upr = "upr",
...
rf_inc_distrito_nomes_dengon <- map(names(rf_distrito_nomes_nowcast), ~ {
  ...
  create_incidence_plot(
    ...
    observed_data = rf_distrito_nomes_nowcast[[distrito_nome]]$observado,
...
tab3_dados <- df_distritos %>% 
  filter(arbo == "dengue" & ano == ano_atual & sem_not %in% weeks_2_plot_full) %>%
...
tab4_dados <- df_distritos %>%
  filter(arbo == "chikon" & ano == ano_atual & sem_not %in% weeks_2_plot_full) %>%
  group_by(sem_not, distrito) %>% 
  summarise(casos_notificados = sum(casos), nivel = sum(nivel)) %>%
  mutate(
    pop = populacoes_distrito_nomes,
